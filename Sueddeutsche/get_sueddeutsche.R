...
rss <- read_xml("https://rss.sueddeutsche.de/rss/Politik")
...
write_xml(rss,file = paste0("./newspaper_rss/Sueddeutsche/RSS_FEEDS/sz_rss_",date_file,"_",time_file,".txt"))
...
if(l == 1){
  ...
  cookies <- readRDS("./newspaper_rss/Sueddeutsche/cookies_sz.rds")
...
result <- tryCatch({
  suppressMessages({
r <- remDr$findElement('xpath', '//*[@class="css-1ccsr7y"]')
date <- unlist(r$getElementText())

r <- remDr$findElement('xpath', '//*[@class="css-1r9juou"]')
title <- unlist(r$getElementText())

c <- remDr$findElements('xpath', '//*[@class=" css-13wylk3"]')
text<-c()
for(k in 1:length(c)){
